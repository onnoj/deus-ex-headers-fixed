 ...
 *	DPLMSG_GETPROPERTYRESPONSE, DPLMSG_SESSIONKEEPALIVE
 */
...
{
	DWORD	dwType;				// Message type
	DWORD	dwRequestID;		// Request ID (zero if no confirmation desired)
	GUID	guidPlayer;			// Player GUID
	GUID	guidPropertyTag;	// Property GUID
	DWORD	dwDataSize;			// Size of data
	BYTE	bPropertyData[1];	// Buffer containing data
...
/*
 *  DPLMSG_SESSIONKEEPALIVE
 *  Standard message sent by the host of a session to a lobby
 *  to indicate that it still exists.
 */
typedef DPLMSG_GENERIC				DPLMSG_SESSIONKEEPALIVE;
typedef DPLMSG_SESSIONKEEPALIVE		*LPDPLMSG_SESSIONKEEPALIVE;
...
/*
 *  The message is a DPLSYS_SESSIONKEEPALIVE message.
 */
#define DPLSYS_SESSIONKEEPALIVE			0x00000009
...
/*
 * DPLPROPERTY_SessionKeepaliveTimeout
 *
 * Request the timeout period that the application should send
 * DPLMSG_SESSIONKEEPALIVE messages to the lobby.
 * 
 * Property data is a single DWORD specifying the timeout in
 * milliseconds.
 */
// {E842A041-D20E-11d0-BA39-00C04FD7ED67}
DEFINE_GUID(DPLPROPERTY_SessionKeepaliveTimeout, 
0xe842a041, 0xd20e, 0x11d0, 0xba, 0x39, 0x0, 0xc0, 0x4f, 0xd7, 0xed, 0x67);
